...
export const Route = createFileRoute("/models/$type")({
  beforeLoad: ({ params }) => {
    const { type } = params;
    // Check if the vehicle model exists
    if (!vehiclesById[type as keyof typeof vehiclesById]) {
  ...
  loader: ({ params }) => {
    const { type } = params;
    const vehicle = vehiclesById[type as keyof typeof vehiclesById];
...
function VehicleDetailPage() {
  ...
  return (
    <div
      className={`transition-opacity duration-500 ${isPageLoaded ? "opacity-100" : "opacity-0"}`}
    >
      {/* Hero section */}
        ...
        <div className="mt-24 bg-gray-100 rounded-xl p-8 md:p-12">
          <div className="text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                href="/kontak"
                className="px-6 py-3 bg-primary text-white rounded-md font-medium text-center hover:bg-primary/90 transition-colors"
              >
                Hubungi Kami
              </a>
              <a
